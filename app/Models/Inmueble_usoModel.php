...
class Inmueble_usoModel extends Model
{
    ...
    protected $allowedFields=['id_inmuebles', 'fecha_apertura', 'fecha_cierre', 'comentario', 'updated_at'];
 
    public function listaInmuebles_usos(){
        $inmuebles=$this ->join('inmuebles','inmuebles_usos.id_inmuebles=inmuebles.id', 'LEFT')
                ->select('inmuebles_usos.id, inmuebles_usos.id_inmuebles, inmuebles_usos.fecha_apertura, 
                inmuebles_usos.fecha_cierre, inmuebles_usos.comentario', 'inmuebles_usos.updated_at', 'inmuebles.nombre as inmueble')
                ->findAll(); 
        return $inmuebles;
    }
}
